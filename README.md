...
$$H_{\mathrm{Ising}} = \sum_i h_i Z_i + \sum_{i<j} J_{ij} Z_i Z_j + \text{offset} \cdot I$$

where:
$$h_i = -\frac{1}{4} \sum_j Q_{ij}^{\mathrm{sym}}, \quad J_{ij} = \frac{Q_{ij}^{\mathrm{sym}}}{4}, \quad \text{offset} = \frac{\sum_{ij} Q_{ij}^{\mathrm{sym}}}{4} + \text{const}$$
...
graph TD
    ...
    subgraph Evaluator["qsynth.evaluator"]
        ...
        I --> N["CircuitVerifier<br/>numerical verification"]
    end
...
│           ├── verifier.py              # Numerical verification (gate identities + circuit equivalence)
...
# With numerical verification (gate identities + circuit-Hamiltonian equivalence)
...
| **Formal verification** | `verify_gate_identities()` checks CNOT²=I, H²=I, Rz(θ)Rz(-θ)=I via matrix products; `verify_numerical()` uses `scipy.linalg.expm` to compare circuit unitary with e^{-iHt} |
...
- **Action**: Append a gate from the gate set $\{H, R_z(\theta), \mathrm{CNOT}, \ldots\}$
- **Reward**: $-d_{\mathrm{circuit}}$ when the target expectation value is achieved within $\varepsilon$
